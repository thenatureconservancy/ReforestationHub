...
afforestation.duration <- 10 # Specify one of 0, 10, 30, 50, or 100
...
nlcd.dat <- paste0(getwd(),'/Raw Raster Data/NLCD_LandCov_Dat/nlcd_2019_land_cover_l48_20210604.img')

###
# Getting soils
soils.dat <- paste0(getwd(),'/Raw Raster Data/Soil Classification/CONUS_SSURGO_soils.tif')

### 
# Potential land cover
bps.dat <- paste0(getwd(),'/Raw Raster Data/LF_BiphysicalPotential/Tif/LC16_BPS_200.tif')
bps.values <- read.csv(paste0(getwd(),'/Raw Raster Data/LF_BiphysicalPotential/CSV_Data/LF16_BPS_200.csv'))
# Used for cross walk
bps.values.old <- read.csv(paste0(getwd(),'/Other Data Inputs/LF_140BPS_01122015.csv'))
# Getting burn age
burn.age.crosswalk <- read.csv(paste0(getwd(),'/Other Data Inputs/BpSMinTreeAge.csv'))
...
bps.type <- read.csv(paste0(getwd(),'/Other Data Inputs/BPS to USFS.csv'))
...
c.seq.rates <- 
  read.csv(paste0(getwd(),'/Other Data Inputs/C sequestration rates.csv'))
...
# Region crosswalk
usfs.crosswalk <- 
  read.csv(paste0(getwd(),'/Other Data Inputs/state_regions.csv')) %>%
...
# State lookup
counties.db <- readOGR(paste0(getwd(),'/Raw Vector Data/Counties/counties_nad6350_with_id.shp'))
states.db <- readOGR(paste0(getwd(),'/Raw Vector Data/State Boundaries/States_reprojected_with_numbers.shp'))
# Merging together
states.counties.db <-
...
forest.dynamics.annual <-
  list.files(paste0(getwd(),'/Raw Raster Data/Forest_Cover_Trends/data'), full.names = TRUE) %>%
  .[!grepl('first|last',.)]
...
connectivity.dat <- paste0(getwd(),'/Raw Raster Data/Connectivity/connectivity_compressed.tif')
...
roads.dat <- paste0(getwd(),"/Raw Raster Data/Roads_Data/roads_raster.tif")
...
states <- paste0(getwd(),'/Raw Raster Data/State Boundaries/states.tif')

# See table here for translation between state id (as in the raster) and county names
states.shp <- readOGR(paste0(getwd(),'/Raw Vector Data/State Boundaries/states_nad6350.shp'))

###
# County boundaries
# from: https://www.census.gov/geographies/mapping-files/time-series/geo/carto-boundary-file.html
# County census data
counties <- paste0(getwd(),'/Raw Raster Data/County Boundaries/counties.tif')
...
nhd <- paste0(getwd(),'/Raw Vector Data/NHD Waterways/nhd_reprojected.shp')

###
# Getting wilderness areas
pad.shp <- readOGR(paste0(getwd(),'/Raw Vector Data/Protected Areas/Reprojected/pad_shape_manager_designation_numeric.shp'))
...
wilderness.areas <- paste0(getwd(),'/Raw Raster Data/Protected_areas/wilderness_areas.tif')
...
burn.crs <- crs(raster(paste0(getwd(),'/Raw Raster Data/Fire_Data/LBA_CU_1999_20200415_C01_V01_BF_L7.tif')))




# Creating folder for managed fire data
if('Managed_Fire_Tiles' %in% list.files(paste0(getwd(),'/Raw Raster Data'))) {
  # Do nothing
} else {
  # Getting raw burn data, and converting it into a tile with data of most recent burn
  dir.create(paste0(getwd(),'/Raw Raster Data/Managed_Fire_Tiles'))
}
...
if(!('CSV_Outputs' %in% list.files(getwd()))) {
  dir.create(paste0(getwd(),"/CSV_Outputs"))
  dir.create(paste0(getwd(),"/Raster_Outputs"))
}
...
#tiles.have <- list.files(paste0(getwd(),'/Raster_Outputs'), pattern = 'total.*ded', ignore.case=TRUE)
...
n = 10 # N = number of cores
# tiles.need <- split(tiles.need, sort(tiles.need%%n))
#cat(tiles.need)
...
#for(x in rev(tiles.need)) {
  ...
  if(!exists('tam.raster')) {
    ...
    tam.raster <- raster(paste0(getwd(),'/Raw Raster Data/Tamalpais_forests/tamalpais_forest_opps.tif'))
  ...
  if(maxValue(bps.tile)<= 31) {
    ...
    if(!identical(crs(bps.tile), crs(raster(paste0(getwd(),"/Raw Raster Data/Roads_Data/roads_raster.tif"))))) {
      tmp.extent <- projectExtent(bps.tile, (raster(paste0(getwd(),"/Raw Raster Data/Roads_Data/roads_raster.tif"))))
      roads.tile <- crop(raster(paste0(getwd(),"/Raw Raster Data/Roads_Data/roads_raster.tif")), tmp.extent)
      roads.tile <- raster::projectRaster(roads.tile, bps.tile, method = 'ngb')
      roads.tile <- raster::resample(roads.tile, bps.tile, method = 'ngb')
    } else {
      roads.tile <- crop(raster(paste0(getwd(),"/Raw Raster Data/Roads_Data/roads_raster.tif")), bps.tile)
      roads.tile <- raster::resample(roads.tile, bps.tile, method = 'ngb')
    }
    ...
    # Waterways data
    if(!identical(crs(bps.tile), crs(raster(paste0(getwd(),"/Raw Raster Data/Waterways/NHD Raster.tif"))))) {
      tmp.extent <- projectExtent(bps.tile, raster(paste0(getwd(),"/Raw Raster Data/Waterways/NHD Raster.tif")))
      nhd.tile <- crop(raster(paste0(getwd(),"/Raw Raster Data/Waterways/NHD Raster.tif")), tmp.extent)
      nhd.tile <- raster::projectRaster(nhd.tile, bps.tile, method = 'ngb')
      nhd.tile <- raster::resample(nhd.tile, bps.tile, method = 'ngb')
    } else {
      nhd.tile <- crop(raster(paste0(getwd(),"/Raw Raster Data/Waterways/NHD Raster.tif")), bps.tile)
      nhd.tile <- raster::resample(nhd.tile, bps.tile, method = 'ngb')
    }
    ...
    if(!identical(crs(bps.tile), crs(raster(paste0(getwd(),"/Raw Raster Data/Protected_areas/pad_ids.tif"))))) {
      tmp.extent <- projectExtent(bps.tile, (raster(paste0(getwd(),"/Raw Raster Data/Protected_areas/pad_ids.tif"))))
      pad.tile <- crop(raster(paste0(getwd(),"/Raw Raster Data/Protected_areas/pad_ids.tif")),bps.tile)
    ...
    flood.tile <-
      list.files(paste0(getwd(),'/Raw Raster Data/Managed_Flood_Tiles'), full.names = TRUE) %>%
    ...
    # Getting fire tile
    burn.tile <- 
      raster(list.files(paste0(getwd(),'/Raw Raster Data/Managed_Fire_Tiles'),
                        full.names = TRUE,
    ...
    tmp.extent <- projectExtent(bps.tile, crs(raster(paste0(getwd(),'/Raw Raster Data/Forest_Cover_Trends/data/VCT_30m_last.tif'))))
    disturb.tile <- crop(raster(paste0(getwd(),'/Raw Raster Data/Forest_Cover_Trends/data/VCT_30m_last.tif')), tmp.extent)
    ...
    opp.df.rasters <- 
      data.frame(r_object = c('forest.opps','flood.tile','con.opps','nhd.tile.convolved','post.disturb.opp', 'soil.tile.crop', 'soil.tile.past'),
                 opp_class = c('total_opportunity_','floodplain_','corridors_','streamside_','postburn_', 'challengingsoils', 'challengingpastures'))
